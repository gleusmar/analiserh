  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex gap-2 border-b border-neutral-200">
        <button className={classNames('px-3 py-2 text-sm', tab==='users' && 'border-b-2 border-neutral-900 font-semibold')} onClick={()=>setTab('users')}>Usuários</button>
        <button className={classNames('px-3 py-2 text-sm', tab==='invites' && 'border-b-2 border-neutral-900 font-semibold')} onClick={()=>setTab('invites')}>Convites</button>
        <button className={classNames('px-3 py-2 text-sm', tab==='audit' && 'border-b-2 border-neutral-900 font-semibold')} onClick={()=>setTab('audit')}>Auditoria</button>
      </div>
            ...
            <div className="space-y-4">
              <div className="flex flex-col sm:flex-row sm:items-center gap-2 justify-between">
                <div className="flex flex-wrap gap-2 items-center">
                  <input value={q} onChange={(e)=>{setUsersPage(1);setQ(e.target.value)}} placeholder="Buscar e-mail" className="rounded-xl border border-neutral-200 bg-white/60 px-3 py-2.5"/>
                  <select value={roleFilter} onChange={(e)=>{setUsersPage(1);setRoleFilter(e.target.value)}} className="rounded-xl border border-neutral-200 bg-white/60 px-3 py-2.5">
                    <option value="all">papel: todos</option>
                    <option value="user">user</option>
                    <option value="admin">admin</option>
                    <option value="gestor-plantoes">gestor-plantoes</option>
                  </select>
                  <select value={statusFilter} onChange={(e)=>{setUsersPage(1);setStatusFilter(e.target.value)}} className="rounded-xl border border-neutral-200 bg-white/60 px-3 py-2.5">
                ...
                {isSuper && (
                  <div className="flex items-center gap-2">
                    <button onClick={()=>setOpenCreate(true)} className="rounded-xl bg-neutral-900 text-white px-3 py-2.5">Criar usuário</button>
                  </div>
                )}
              </div>
                            ...
                            {canAdmin ? (
                              <div className="flex items-center gap-2">
                                <select
                                  ...
                                  className="rounded-lg border border-neutral-200 bg-white/60 px-2 py-1.5 min-w-[220px]"
                                >
                              ...
                              <span className="text-sm text-neutral-700">{u.collaborator_id ? collabNameById(u.collaborator_id) : '-'}</span>
                            )}
                          </td>
                          <td className="py-2">
                            {isSuper ? (
              ...
              <div className="flex items-center justify-between text-sm text-neutral-600">
                ...
                <div className="inline-flex items-center gap-2">
                  <select value={usersPageSize} onChange={(e)=>{setUsersPage(1);setUsersPageSize(parseInt(e.target.value)||10)}} className="rounded-lg border border-neutral-200 bg-transparent px-2 py-1">
                    <option value={10}>10</option>
                    <option value={20}>20</option>
                    <option value={50}>50</option>
                  </select>
                  <button disabled={usersPage<=1} onClick={()=>setUsersPage(p=>Math.max(1,p-1))} className="px-2 py-1 rounded-lg border border-neutral-200 disabled:opacity-50">Anterior</button>
                  <button disabled={usersPage*usersPageSize>=usersTotal} onClick={()=>setUsersPage(p=>p+1)} className="px-2 py-1 rounded-lg border border-neutral-200 disabled:opacity-50">Próxima</button>
                </div>
              </div>
              <CreateUserModal
                open={openCreate}
            ...
            <div className="space-y-6">
              ...
              <form onSubmit={onInvite} className="glass rounded-xl p-4 space-y-3">
                <div className="grid sm:grid-cols-3 gap-2">
                  <input type="email" required value={form.email} onChange={(e)=>setForm((f)=>({...f,email:e.target.value}))} placeholder="E-mail para convite" className="rounded-xl border border-neutral-200 bg-white/60 px-3 py-2.5 outline-none focus:ring-4 ring-sky-100"/>
                  <select value={form.role} onChange={(e)=>setForm((f)=>({...f,role:e.target.value}))} className="rounded-xl border border-neutral-200 bg-white/60 px-3 py-2.5">
                    <option value="user">user</option>
                    <option value="admin">admin</option>
                    <option value="gestor-plantoes">gestor-plantoes</option>
                  </select>
                  <button disabled={creating} className="rounded-xl bg-neutral-900 text-white px-3 py-2.5">{creating? 'Enviando...' : 'Gerar convite'}</button>
                </div>
                {justCreatedInvite && (
                  <div className="text-sm text-neutral-700">
                    Link do convite: 
                    <button type="button" onClick={()=>copyLink(justCreatedInvite)} className="ml-2 underline text-sky-600">copiar</button>
                  </div>
                )}
              </form>
              )}
                      ...
                      <tr key={inv.id} className="border-t border-neutral-200">
                        ...
                        <td className="py-2">
                          <div className="inline-flex gap-2">
                            <button onClick={()=>copyLink(inv)} className="px-2 py-1 rounded-lg border border-neutral-200">Copiar link</button>
                            <button disabled={inv.status!=='pending'} onClick={()=>onRevoke(inv)} className="px-2 py-1 rounded-lg border border-neutral-200 disabled:opacity-50">Revogar</button>
                          </div>
                        </td>
                      </tr>
                    ))}
              ...
              <div className="flex items-center justify-between text-sm text-neutral-600">
                ...
                <div className="inline-flex items-center gap-2">
                  <select value={auditPageSize} onChange={(e)=>{setAuditPage(1);setAuditPageSize(parseInt(e.target.value)||20)}} className="rounded-lg border border-neutral-200 bg-transparent px-2 py-1">
                    <option value={10}>10</option>
                    <option value={20}>20</option>
                    <option value={50}>50</option>
                  </select>
                  <button disabled={auditPage<=1} onClick={()=>setAuditPage(p=>Math.max(1,p-1))} className="px-2 py-1 rounded-lg border border-neutral-200 disabled:opacity-50">Anterior</button>
                  <button disabled={auditPage*auditPageSize>=auditTotal} onClick={()=>setAuditPage(p=>p+1)} className="px-2 py-1 rounded-lg border border-neutral-200 disabled:opacity-50">Próxima</button>
                </div>
              </div>
            </div>
          )}
          ...
          {tab === 'audit' && (
            <div className="space-y-4">
              <div className="flex flex-wrap items-center gap-2">
                <input value={auditQ} onChange={(e)=>{setAuditPage(1);setAuditQ(e.target.value)}} placeholder="Buscar ator/alvo" className="rounded-xl border border-neutral-200 bg-white/60 px-3 py-2.5"/>
                <select value={auditAction} onChange={(e)=>{setAuditPage(1);setAuditAction(e.target.value)}} className="rounded-xl border border-neutral-200 bg-white/60 px-3 py-2.5">
