...
function extractMappedValues(text) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    // Capture the first 3 digits at line start (or after space). Handles '5141ª PARCELA' cases.
    const m = raw.match(/(?:^|\s)(\d{3})/)
    if (!m) continue
    // If no amount on this code line, scan forward until next code or TOTAL
    if (!amount) {
      for (let j = i + 1; j < lines.length; j++) {
        const ln = lines[j]
        const nn = normalizeText(ln)
